...
- Отладочные логи включаются через `custom_components.ozon_package_tracker`
  (**с этим префиксом**, а не просто `ozon_package_tracker`, — так называется
  логгер любого файла внутри `custom_components/<домен>/`):
  ```yaml
  logger:
    default: warning
    logs:
      custom_components.ozon_package_tracker: debug
  ```
  В debug‑режиме на каждой проверке посылки видно, был ли реально сделан
  запрос, какой статус получен и изменился ли он по сравнению с прошлым разом.
