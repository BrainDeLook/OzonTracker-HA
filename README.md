...
Если в журнале HA видно `bff-api: HTTP 403; page: HTTP 403` — запросы режет
антибот-защита Ozon. Интеграция сама пытается «прогреться»: заходит на
страницу трекинга с браузерными заголовками и собирает выданные куки. Если
этого недостаточно, подставьте куки своего браузера:

1. Откройте [tracking.ozon.ru](https://tracking.ozon.ru/?track=ваш-трек) в
   браузере на любом устройстве.
2. DevTools (**F12**) → вкладка **Network** → кликните запрос с вашим
   трек-номером → **Headers** → секция **Request Headers** → скопируйте
   значение заголовка `cookie` целиком.
3. В HA: *Настройки → Устройства и службы → Ozon Package Tracker →
   Настроить* → вставьте скопированное в поле **«Заголовок Cookie»**.

Куки антибота живут долго, но не вечно: если 403 вернётся через
несколько недель — повторите шаги. Также убедитесь, что HA выходит в
интернет с российского IP (не через зарубежный VPN/VPS): в этом случае
куки могут не помочь.
