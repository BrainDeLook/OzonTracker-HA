...
- Интеграция использует **неофициальный** эндпоинт страницы
  tracking.ozon.ru; Ozon может изменить его в любой момент. Клиент
  (`custom_components/ozon_package_tracker/api.py`) пробует несколько
  вариантов запроса (GET/POST API, разбор JSON из HTML-страницы) и не
  привязан к точной схеме ответа, но при изменениях на стороне Ozon может
  потребоваться правка `api.py`.
